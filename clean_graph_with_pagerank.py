    ...
    with open(dot_filename, 'r') as file:
        for line in file:
            ...
            if node_match:
                ...
                nodes[node_id] = {'label': label, 'year': year, 'citation_count': citation_count, 'url': url,
                                  'page_rank': page_rank}
            else:
...
def filter_and_write_dot_file(nodes, edges, output_dot_filename):
    with open(output_dot_filename, 'w') as file:
        ...
        for node_id, node in nodes.items():
            # if node['page_rank'] > 0:
            file.write(
                f'{node_id}[label="{node["label"]}", year="{node["year"]}", citationCount="{node["citation_count"]}", url="{node["url"]}", pageRank="{node["page_rank"]}"];\n')
...
if __name__ == "__main__":
    ...
    output_dot_filename = 'data/output.dot'
